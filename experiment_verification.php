...
if ($appointmentsResult != false) {
	if (mysqli_num_rows($appointmentsResult) > 0) {
		while ($row = mysqli_fetch_assoc($appointmentsResult)) {
			...
			$results = array($row['Study Name'], $row['Proctor Name'], $row['Subject Name'], date_format(date_create($row['Date']), 'M d, Y'), date_format(date_create($row['Expected Start Time']), "H:i") , date_format(date_create($row['Expected End Time']), "H:i"));
